...
void crud::deleteRecord(std::fstream &data)
{
	...
	// 3. kita cek data yang ada dari file data.dat, kalau ada data
	...
	// 4. kita pindahkan data dari file sementara ke data.dat
	size = crud::getDataSize(dataSementara);
	data.close();
	data.open("data.dat", std::ios::trunc | std::ios::out | std::ios::binary);
	data.close();
...
void crud::checkDatabase(std::fstream &data)
{
	...
	else
	{
		...
		data.close();
		data.open("data.dat", std::ios::trunc | std::ios::out | std::ios::in | std::ios::binary);
	}
}
